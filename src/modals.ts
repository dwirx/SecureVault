	...
	onOpen() {
		...
		if (this.settings.enableKeyFile) {
			new Setting(contentEl)
				...
				.setDesc('Select key file for additional security')
				.addButton(btn => btn
					...
					.onClick(async () => {
						...
						input.onchange = async (e: any) => {
							...
							if (file) {
								...
								if (validateKeyFile(text)) {
									this.keyFileContent = text.trim();
									new Notice('✅ Key file loaded successfully');
								} else {
	...
	onOpen() {
		...
		if (this.settings.enableKeyFile) {
			const keyFileSetting = new Setting(contentEl)
				.setName('🔑 Key File (Optional)')
				.setDesc('Additional security layer');
			...
			keyFileSetting.addButton(btn => btn
				...
				.onClick(async () => {
					...
					a.href = url;
					a.download = `securevault-${Date.now()}.key`;
					a.click();
					URL.revokeObjectURL(url);
					
					new Notice('✅ Key file generated and downloaded! Keep it safe!', 8000);
				}));
				...
				.onClick(async () => {
					...
					input.onchange = async (e: any) => {
						...
						if (file) {
							...
							if (validateKeyFile(text)) {
								this.keyFileContent = text.trim();
								new Notice('✅ Key file loaded');
							} else {
								new Notice('❌ Invalid key file');
							}
						}
					};
					input.click();
